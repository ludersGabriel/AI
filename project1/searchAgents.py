...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        # all we need is the starting pacman position and
        # the corners visited status
    ...
    def getSuccessors(self, state: Any):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                ...
                successors.append(
                    ((nextx, nexty) + tuple(visited), action, 1.0)
                )
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    for i, corner in enumerate(corners):
        if not state[i + 2]:
            ...
            # find the manhattan distance to the corner
            # and add it to the dominant heuristic
            currentH = util.manhattanDistance(state, corner)
            if currentH > dominantH:
                dominantH = currentH
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    for food in foodGrid.asList():
        mazeDist = problem.heuristicInfo.get((position, food), None)
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState: pacman.GameState):
        ...
        for f in food.asList():
            distance = util.manhattanDistance(f, startPosition)
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state: Tuple[int, int]):
        ...
        "*** YOUR CODE HERE ***"
        # check if the current state is a food, since the
        # only requirement is to find any food
